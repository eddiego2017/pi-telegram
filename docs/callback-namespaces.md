...
- Do not use `pi-telegram` owned prefixes: `tgbtn:`, `menu:`, `model:`, `thinking:`, `status:`, `queue:`, `resume:`, `session:`, `tree:`, `settings:`, `section:`. Current app navigation uses `menu:`; `status:` remains reserved for legacy/owned status callbacks but is not emitted by current UI. `resume:` is owned by the session resume/delete picker, `session:` by the current-session center/history viewer, `tree:` by the session-tree rewind menu, `section:` by the Extension Sections platform (0.10.0+), documented in [Extension Sections](./extension-sections.md), and `settings:` by the built-in Settings submenu.
