...
- `/tree` for opening a Telegram-native active-path prompt history with user/custom prompts only, an inactive-branches toggle, detail views, strict idle rewind/switch gating, and outcome notices driven by internal `/telegram-tree-exec` plus `ctx.navigateTree()`
