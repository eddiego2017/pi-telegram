...
- `/delete` for opening the same session picker directly in one-session delete mode: numeric row buttons open an explicit `🗑 Delete` / Back / Cancel confirmation, and success offers Delete another or Resume without exposing the batch-selection controls
