...
- `/tab` for the opt-in concurrent tab MVP. When `concurrentTabs.enabled` is true, `/tab new <name>`, `/tab <name>`, `/tab status`, `/tab abort`, `/tab close` for the active tab, `/tab close <name>` for a named tab, and `/tab restart <name>` are handled by the parent Telegram bridge while each tab owns a separate RPC child process. Workers are always launched with `--no-extensions`, so they cannot load pi-telegram and start another poller. Normal Telegram prompts route directly to the active tab; inactive tabs accumulate text and send compact completion notices. Switching tabs replays the selected tab's latest user turn with the full response path, including thinking blocks, assistant tool calls, tool results, visible replies, and replayable image attachments.
