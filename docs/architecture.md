...
- `model` / `menu-model` / `menu-thinking` / `menu-status` / `menu` / `menu-queue` / `menu-session` / `menu-tree` / `menu-settings` / `commands`: model identity/thinking levels, scoped model resolution, in-flight switching, model/thinking/status/queue/session/tree-rewind/settings menu UI, inline application callback composition, slash commands, and bot command registration.
...
- `/session` for opening a Telegram-native current-session center with compact stats, latest preview, and active-branch history pagination rendered as a narrow user/assistant/custom table without tool-call/tool-result noise or link previews
- `/tree` for opening a Telegram-native session-tree rewind menu with chat/all filters, paginated entry browsing, detail view, strict idle rewind gating, default-summary support, and outcome notices driven by internal `/telegram-tree-exec` plus `ctx.navigateTree()`
