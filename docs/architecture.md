...
- `api`: Bot API transport shapes/helpers, retries, outbound delivery limiting/backoff, stale typing/preview dropping while limited, file download, temp-dir lifecycle, inbound limits, chat actions, lazy bot-token clients, runtime error recording, and the `TELEGRAM_API_BASE` constant for the Bot API endpoint.
...
- `workspaces` / `rpc-child` / `workspace-manager`: opt-in concurrent Telegram workspaces; owns workspace registry state, `/workspace` parsing/status formatting, forum-topic workspace source metadata, sticky one-to-one topic/workspace worker supervision using isolated `pi --mode rpc --no-extensions` children, and text-first routing to active or topic-bound workspace workers.
...
- `/queue` for opening the queue section of the inline application menu; the same section is reachable from the status/main menu and supports top-anchored Back navigation, Priority/Normal workspaces, and cancellation
- `/resume` for opening a paginated list of previous sessions in the current cwd; session previews render as mobile-friendly two-line message-body items (`① age · Nmsg` followed by the summary) with `Nmsg` recomputed from visible user/assistant messages so hidden thinking, assistant tool-call-only turns, and tool results do not inflate the count; compact page-local numeric inline buttons are backed by stable global callback indices; the same menu's `Manage 🗑` mode toggles multi-select delete checkboxes, `Select page` selects only the current page of deletable rows, and the explicit confirmation step reuses the trash/unlink batch delete path while refusing the current active session; when concurrent workspaces are enabled, `/resume` keeps the same global cwd session listing but switches the selected session into the active workspace via worker RPC `switch_session`, with a parent-side worker restart if the child reports stale session state after a successful switch
...
- `/llm [tokens...]` for listing available `provider/id` models with the active model marked by `🟢`; filtered multi-match replies keep the same marker, and single matches switch the parent session or active workspace model when idle/switchable
- `/workspace` for the opt-in concurrent workspace MVP. When `concurrentWorkspaces.enabled` is true, `/workspace new <name>`, `/workspace <name>`, `/workspace status`, `/workspace abort`, `/workspace close` for the active workspace, `/workspace close <name>` for a named workspace, and `/workspace restart <name>` are handled by the parent Telegram bridge while each workspace owns a separate RPC child process. Workers are always launched with `--no-extensions`, so they cannot load pi-telegram and start another poller. Normal Telegram prompts route directly to the active workspace; inactive workspaces accumulate text and send compact completion notices. While a worker is compacting, new prompts for that workspace are not refused: they are appended to a per-runtime `pendingCompactionTurns` queue, acknowledged with a `(compaction in progress, N waiting)` notice, and flushed in arrival order once compaction completes (first delivery uses `prompt`, the rest `followUp`). If compaction fails, or the workspace is aborted/stopped, the queued turns are dropped and each sender is notified. The `/workspace` dashboard shows topic/workspace worker state (`worker idle`, `worker running`, or `worker not started`), workspace age, session name, and latest-message summaries. Switching workspaces replays the selected workspace's latest 5 session messages, including thinking blocks, assistant tool calls, tool results, visible replies, and replayable image attachments. When `concurrentWorkspaces.topicBinding.enabled` is true, forum-topic prompt routing overrides the manual active workspace for normal prompts: General/no thread id uses the internal `default` record but displays as `General`, non-General `message_thread_id` lazily creates/finds a workspace whose `source` stores the full chat/topic identity, and worker output is treated as delivery-active for that topic even if the global active workspace points elsewhere. The forum-native product policy is sticky one-to-one topic workers with Option A first-prompt spawn: topic service create/edit/reopen records metadata only, first prompt or worker-scoped command starts exactly one worker for that topic, subsequent topic work reuses it, and topic close disposes it while preserving the session JSONL. Workers are not reused across topics; in forum-native mode `maxWorkers` is treated as `maxWorkspaces`, so capacity overflow is a clear refusal instead of stopping another open topic's worker. A shared thread normalizer maps missing `message_thread_id` to General and leaves room for a future explicit General thread id. Topic service messages are swallowed, create/edit/reopen update workspace metadata, close marks the runtime closing, stops typing/stream timers, disposes the worker, removes the record, ignores late child events, and preserves the session JSONL. Topic `/resume` enforces one open workspace per session by canonical session-file identity with a `sessionId` fallback and checks live worker state before both resume and prompt dispatch. In `topicBinding.native` mode, Bot Commands and `/start` help hide `/workspace`, scoped controls use current-topic wording, and ordinary prompt start/follow-up/busy/failure/empty-prompt/abort plus inactive-completion notices, `/compact`/`/new`/`/clone` session lifecycle replies, and worker lifecycle errors say topic/workspace instead of workspace names while keeping `/workspace` routable as a hidden operator/debug dashboard. `/topic orphans` lists proven topic orphans, errored topic records, and suspected records with no current worker; Bot API `message thread not found` / `topic not found` delivery failures record orphan proofs, and `/topic cleanup` removes only those proven local records while preserving session files. Workspace-aware command/menu APIs prefer the ambient topic scope; `/workspace` remains a global/manual dashboard.
...
- Structured debug logging is separate from `/telegram-status`: the bridge emits redacted JSON lines to stdout by default for collection by Kubernetes logging stacks such as Alloy/Loki. Events cover polling, update routing, Telegram API requests/responses including `answerCallbackQuery` and `editMessageText`, queue dispatch, agent/tool lifecycle, and concurrent-workspace worker events. Workspace workers emit semantic events (`telegram.workspace.agent.start`, `telegram.workspace.first_output`, `telegram.workspace.agent.end`, `telegram.workspace.turn.summary`) so latency can be measured without parsing every raw worker delta; high-frequency `message_update` bodies plus final `message_end` / `agent_end` payloads are omitted from the generic worker-event log to control volume and avoid persisting reasoning signatures. Request/response bodies are included in redacted form by default, can be disabled with `PI_TELEGRAM_DEBUG_BODIES=0` / `debug.includeBodies: false`, and remain credential-redacted plus size-limited even in raw mode.
