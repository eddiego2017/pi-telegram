...
- `tabs` / `rpc-child` / `tab-manager`: opt-in concurrent Telegram tabs; owns tab registry state, `/tab` parsing/status formatting, forum-topic tab source metadata, separate durable workspace (`maxTabs`) and live worker (`maxWorkers`) capacity, isolated `pi --mode rpc --no-extensions` worker supervision, and text-first routing to active or topic-bound tab workers.
...
- `/tab` for the opt-in concurrent tab MVP. When `concurrentTabs.enabled` is true, `/tab new <name>`, `/tab <name>`, `/tab status`, `/tab abort`, `/tab close` for the active tab, `/tab close <name>` for a named tab, and `/tab restart <name>` are handled by the parent Telegram bridge while each tab owns a separate RPC child process. Workers are always launched with `--no-extensions`, so they cannot load pi-telegram and start another poller. Normal Telegram prompts route directly to the active tab; inactive tabs accumulate text and send compact completion notices. The `/tab` dashboard shows hot worker capacity as `Workers: hot/maxWorkers hot` above the active/current row, and each row marks whether its worker is hot or currently has no live worker. Switching tabs replays the selected tab's latest 5 session messages, including thinking blocks, assistant tool calls, tool results, visible replies, and replayable image attachments. When `concurrentTabs.topicBinding.enabled` is true, forum-topic prompt routing overrides the manual active tab for normal prompts: General/no thread id uses the internal `default` record but displays as `General`, non-General `message_thread_id` lazily creates/finds a tab whose `source` stores the full chat/topic identity, and worker output is treated as delivery-active for that topic even if the global active tab points elsewhere. A shared thread normalizer maps missing `message_thread_id` to General and leaves room for a future explicit General thread id. Topic service messages are swallowed, create/edit/reopen update tab metadata, close marks the runtime closing, stops typing/stream timers, disposes the worker, removes the record, ignores late child events, and preserves the session JSONL. Topic `/resume` enforces one open workspace per session by canonical session-file identity with a `sessionId` fallback and checks hot worker state before both resume and prompt dispatch. `concurrentTabs.maxTabs` limits durable workspace/topic records, while optional `concurrentTabs.maxWorkers` limits concurrently hot RPC child workers and defaults to `maxTabs` when unset. In `topicBinding.native` mode, Bot Commands and `/start` help hide `/tab`, scoped controls use current-topic wording, and ordinary prompt start/follow-up/busy/failure/empty-prompt/abort plus inactive-completion notices, `/compact`/`/new`/`/clone` session lifecycle replies, and worker lifecycle errors say topic/workspace instead of tab names while keeping `/tab` routable as a hidden operator/debug dashboard. `/topic orphans` lists proven topic orphans, errored topic records, and suspected cold records; Bot API `message thread not found` / `topic not found` delivery failures record orphan proofs, and `/topic cleanup` removes only those proven local records while preserving session files. Tab-aware command/menu APIs prefer the ambient topic scope; `/tab` remains a global/manual dashboard.
