...
- `/tree` for opening a Telegram-native active-path prompt history with user/custom prompts only, a detail view, strict idle rewind gating, and outcome notices driven by internal `/telegram-tree-exec` plus `ctx.navigateTree()`
