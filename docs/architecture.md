...
- `tabs` / `rpc-child` / `tab-manager`: opt-in concurrent Telegram tabs; owns tab registry state, `/tab` parsing/status formatting, forum-topic tab source metadata, sticky one-to-one topic/tab worker supervision using isolated `pi --mode rpc --no-extensions` children, and text-first routing to active or topic-bound tab workers.
...
5. Await and verify concurrent-tab final stream delivery before suppressing the normal full-reply fallback; if the final edit/send is not confirmed, send the complete assistant text as a normal reply

Draft streaming can remain as a plain-text fallback path, but rich Telegram previews are driven through editable messages and stable-block snapshot selection. Concurrent-tab stream operations carry the originating turn id and delivery target so delayed edits from an old turn cannot block or mutate the next turn's stream state; aborted partial previews are marked with `[aborted]` when possible.
...
- `/tab` for the opt-in concurrent tab MVP. When `concurrentTabs.enabled` is true, `/tab new <name>`, `/tab <name>`, `/tab status`, `/tab abort`, `/tab close` for the active tab, `/tab close <name>` for a named tab, and `/tab restart <name>` are handled by the parent Telegram bridge while each tab owns a separate RPC child process. Workers are always launched with `--no-extensions`, so they cannot load pi-telegram and start another poller. Normal Telegram prompts route directly to the active tab; inactive tabs accumulate text and send compact completion notices. While a worker is compacting, new prompts for that tab are not refused: they are appended to a per-runtime `pendingCompactionTurns` queue, acknowledged with a `(compaction in progress, N waiting)` notice, and flushed in arrival order once compaction completes (first delivery uses `prompt`, the rest `followUp`). If compaction fails, or the tab is aborted/stopped, the queued turns are dropped and each sender is notified. The `/tab` dashboard shows topic/tab worker state (`worker idle`, `worker running`, or `worker not started`), tab age, session name, and latest-message summaries. Switching tabs replays the selected tab's latest 5 session messages, including thinking blocks, assistant tool calls, tool results, visible replies, and replayable image attachments. When `concurrentTabs.topicBinding.enabled` is true, forum-topic prompt routing overrides the manual active tab for normal prompts: General/no thread id uses the internal `default` record but displays as `General`, non-General `message_thread_id` lazily creates/finds a tab whose `source` stores the full chat/topic identity, and worker output is treated as delivery-active for that topic even if the global active tab points elsewhere. The forum-native product policy is sticky one-to-one topic workers with Option A first-prompt spawn: topic service create/edit/reopen records metadata only, first prompt or worker-scoped command starts exactly one worker for that topic, subsequent topic work reuses it, and topic close disposes it while preserving the session JSONL. Workers are not reused across topics; in forum-native mode `maxWorkers` is treated as `maxTabs`, so capacity overflow is a clear refusal instead of stopping another open topic's worker. A shared thread normalizer maps missing `message_thread_id` to General and leaves room for a future explicit General thread id. Topic service messages are swallowed, create/edit/reopen update tab metadata, close marks the runtime closing, stops typing/stream timers, disposes the worker, removes the record, ignores late child events, and preserves the session JSONL. Topic `/resume` enforces one open workspace per session by canonical session-file identity with a `sessionId` fallback and checks live worker state before both resume and prompt dispatch. In `topicBinding.native` mode, Bot Commands and `/start` help hide `/tab`, scoped controls use current-topic wording, and ordinary prompt start/follow-up/busy/failure/empty-prompt/abort plus inactive-completion notices, `/compact`/`/new`/`/clone` session lifecycle replies, and worker lifecycle errors say topic/workspace instead of tab names while keeping `/tab` routable as a hidden operator/debug dashboard. `/topic orphans` lists proven topic orphans, errored topic records, and suspected records with no current worker; Bot API `message thread not found` / `topic not found` delivery failures record orphan proofs, and `/topic cleanup` removes only those proven local records while preserving session files. Tab-aware command/menu APIs prefer the ambient topic scope; `/tab` remains a global/manual dashboard.
