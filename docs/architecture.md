...
- `tabs` / `rpc-child` / `tab-manager`: opt-in concurrent Telegram tabs; owns tab registry state, `/tab` parsing/status formatting, forum-topic tab source metadata, isolated `pi --mode rpc --no-extensions` worker supervision, and text-first routing to active or topic-bound tab workers.
