...
- `/start`, `/help`, and `/status` open the unified command-help/status-row/control menu; `/model`, `/thinking`, and `/queue` jump to sections directly; visible bot commands are `/start`, `/compact`, `/reload`, `/new`, `/clone`, `/resume`, `/name`, `/llm`, `/next`, `/continue`, `/abort`, `/stop`
