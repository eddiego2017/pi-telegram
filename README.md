...
Forum topic binding is an additional opt-in layer for Telegram forum supergroups. Enable it with `concurrentTabs.topicBinding.enabled: true`; General/no `message_thread_id` routes to the existing `default` tab, while each non-General topic lazily creates a compact `tg-…` tab keyed by chat id plus topic id. Topic-bound prompt output, streaming previews, errors, and `typing` actions are delivered back into that same topic without changing the manual `/tab` active selection. Topic create/edit/reopen service messages sync tab metadata, topic titles seed the bound session display name and are applied to new/resumed topic sessions, topic close can dispose and remove the bound worker tab while preserving the session file, and service messages are swallowed rather than forwarded as prompts. Tab-aware controls such as `/llm`, `/model`, `/new`, `/resume`, `/session`, `/tree`, `/name`, `/compact`, `/abort`, and `/stop` prefer the ambient topic tab; `/tab` itself remains the global/manual dashboard. For native ordinary topic messages in groups, keep using an operator-controlled private forum group where the bot is admin, privacy mode is disabled and the bot was re-added, or messages explicitly address the bot.
