...
- **`/session`**: Open a Telegram-native session center for the current π session: compact name/file/id/message/token/cost/context stats, latest chat preview, and active-branch chat-history pagination. History rows render as a narrow HTML row table directly in the panel, show how much tool noise is hidden, and do not trigger Telegram link previews.
- **`/tree`**: Open a Telegram-native session-tree rewind menu. Pick an entry, inspect its text, then rewind there with or without π's default branch summary. Rewind is allowed only while π and the Telegram queue are idle. When a selected user/custom entry returns prompt text, Telegram cannot prefill π's editor, so the bridge sends the text back for copy/edit/resend.
...
The inline application menu is the primary operator surface. It exposes status, prompt-template commands, model selection, thinking level selection, settings, queue inspection/mutation, session resume/delete flows, the `/session` session-center/history view, and `/tree` session-tree rewind: a Telegram-shaped subset of the important handles normally available from the CLI. A typical control loop stays inside Telegram: open `/start`, inspect status, jump into Queue, inspect current session history, rewind or branch from a previous point, delete stale work, switch model, resume or prune old sessions, return to the main menu, and keep the π session running without touching the terminal.
