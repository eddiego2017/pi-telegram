...
- **`/session`**: Open a Telegram-native session center for the current π session: compact name/file/id/message/token/cost/context stats, latest chat preview, and active-branch history pagination. History rows render directly in the HTML panel and skip tool-call/tool-result noise by default.
