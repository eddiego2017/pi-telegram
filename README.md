...
- **`/start`**: Pair the first Telegram user when needed, register bot commands, and open the inline application menu with command help, prompt-template commands, status rows, model controls, thinking controls, settings, and queue controls. In forum-native mode, the visible command menu and `/start` help omit `/workspace` and use topic/current-topic wording for scoped controls.
- **`/compact`**: Start session compaction when the session is idle; Telegram shows the native typing indicator while compaction is running. In forum-native mode, compaction replies describe the current topic/workspace instead of a hidden workspace-backed session.
...
- **`/llm [tokens...]`**: List available LLM models (`provider/id`), marking the active model with `🟢`. With tokens, filter by case-insensitive AND substring match on the model id; if exactly one model matches, switch the active model and reply `Model switched to provider/id`. When concurrent workspaces are enabled this switches the active workspace's child worker; otherwise it switches the parent session. Multiple matches list the filtered subset; no match replies `No models match: <tokens>`.
- **`/workspace`**: Manage the opt-in concurrent workspace MVP. Enable it in `telegram.json` with `{"concurrentWorkspaces":{"enabled":true,"maxWorkspaces":10,"inactiveNotify":true}}`, then use `/workspace` for the inline workspace dashboard or text commands such as `/workspace new A`, `/workspace B`, `/workspace rename A Work`, `/workspace status`, `/workspace abort`, `/workspace close` for the active workspace, and `/workspace close A` for a named workspace. The dashboard shows active/running/unread workspaces with workspace age, topic-owned worker state (`worker idle`, `worker running`, or `worker not started`), message count, session name, and latest message preview instead of model ids; it switches workspaces with buttons, keeps confirmed close controls, and includes `Manage 🗑` for multi-select workspace close with an explicit confirmation. Each workspace runs a separate `pi --mode rpc --no-extensions` worker and keeps its own active session pointer while sessions remain in the normal shared cwd session pool, so a long run in one workspace does not block prompts sent to another workspace. Switching with `/workspace <name>` or a dashboard workspace button automatically replays that workspace's latest 5 session messages, including thinking blocks, tool calls, tool results, agent replies, and replayable image attachments. The parent bridge owns Telegram delivery and streams active-workspace answer text, thinking previews, tool-call previews, final Markdown replies, and native Telegram `typing` chat actions from worker RPC activity. `/llm`, `/model`, `/new`, `/resume`, `/session`, and `/tree` follow the active workspace, or the ambient forum-topic workspace when topic binding is enabled and the command is invoked inside a topic. In forum-native mode, `/workspace` is hidden from the bot command menu and `/start` help; it remains an operator/debug dashboard. Worker workspaces still do not have `telegram_attach` yet, so generated files are saved locally and should be referenced by path.
- **`/next`**: Dispatch the next queued turn, aborting π first if needed.
- **`/continue`**: Enqueue a priority `continue` prompt.
- **`/abort`**: Abort the active run without touching the queue. When concurrent workspaces are enabled, this targets the active workspace worker, or the ambient forum-topic workspace when topic binding is enabled.
- **`/stop`**: Abort the active run and clear waiting Telegram queue items. When concurrent workspaces are enabled, this targets the active workspace worker, or the ambient forum-topic workspace when topic binding is enabled, and still clears the Telegram queue.

Hidden compatibility shortcuts: `/help` and `/status` open the main application menu, `/model` opens model controls, `/thinking` opens reasoning controls, `/queue` opens queue controls, `/settings` opens bridge settings, and `/topic orphans` / `/topic cleanup` expose forum-topic repair diagnostics without creating or switching topics. When concurrent workspaces are enabled, `/model` opens the active workspace's model picker and applies model picks to that workspace's child worker while the workspace is idle.
...
### Concurrent workspaces MVP

Concurrent workspaces are disabled by default. When `concurrentWorkspaces.enabled` is true, normal Telegram prompts go to the active workspace instead of the single-session queue. `/workspace` commands stay parent-owned and responsive while workers run; bare `/workspace` opens an inline dashboard for switching workspaces, topic-owned worker state (`worker idle`, `worker running`, or `worker not started`), workspace age/message/name/latest-message summaries, confirmed close controls, and `Manage 🗑` multi-select close. `/workspace close` closes the active non-default workspace directly, while `/workspace close <name>` still closes a named workspace. `/workspace rename [old-name] <new-name>` renames non-default workspaces without discarding their session file, `/llm [tokens...]` plus `/model` switch the active workspace's child model when that workspace is idle, `/new` and `/resume` mutate the active workspace's worker session, and `/session` plus `/tree` read the active workspace's worker session/history after `/workspace <name>` switches. This active-workspace behavior has been smoke-tested in Telegram across isolated workspaces. Each workspace uses an isolated RPC child launched with `--no-extensions`, preventing a second Telegram poller from starting inside a worker. The parent bridge sends native Telegram `typing` actions for the active running workspace and stops them when that workspace finishes, is aborted, errors, exits, or is switched away. Inactive workspaces collect output quietly and send a compact completion notice; switching back with `/workspace <name>` or a dashboard workspace button automatically replays the latest 5 session messages, including thinking, tool calls, tool results, agent replies, and replayable image attachments.

Forum topic binding is an additional opt-in layer for Telegram forum supergroups. Enable it with `concurrentWorkspaces.topicBinding.enabled: true`; General/no `message_thread_id` routes to the existing internal `default` workspace but is displayed as `General`, while each non-General topic lazily creates a compact `tg-…` workspace keyed by chat id plus topic id. In forum-native mode the intended policy is sticky one-to-one topic workers: a topic create/edit/reopen service message records metadata only, the first prompt or worker-scoped command in that topic starts exactly one worker for that topic, subsequent prompts reuse it, and topic close disposes it while preserving the session file. Workers are not reused across topics; in forum-native mode `maxWorkers` is treated as `maxWorkspaces`, so capacity overflow is a clear refusal instead of stopping another open topic's worker. Topic-bound prompt output, streaming previews, errors, and `typing` actions are delivered back into that same topic without changing the manual `/workspace` active selection; in forum-native mode, ordinary prompt acknowledgements, abort results, failures, inactive-completion notices, `/compact`/`/new`/`/clone` session lifecycle replies, and worker lifecycle errors say topic/workspace instead of exposing internal workspace names. Topic create/edit/reopen service messages sync workspace metadata, topic titles seed the bound session display name and are applied to new/resumed topic sessions. Topic close marks the runtime closing, stops typing/stream flushes, disposes the worker, removes the bound workspace, ignores late child events, and preserves the session file. A session can be attached to only one open topic/workspace at a time; `/resume` canonicalizes the target session path and refuses conflicts with a message naming the owning workspace. In forum-native mode, visible Bot Commands and `/start` help hide `/workspace` and describe scoped controls as current-topic operations; `/workspace` still routes as a hidden operator/debug dashboard. `/topic orphans` lists proven topic orphans, errored topic records, and suspected topic records with no current worker; `/topic cleanup` removes only records proven orphan by clear Bot API missing-topic failures, preserves session files, and leaves no-proof records untouched. Service messages are swallowed rather than forwarded as prompts. Thread handling is normalized centrally: missing `message_thread_id` means General today, and a future known General thread id can be configured without changing callers. Workspace-aware controls such as `/llm`, `/model`, `/new`, `/resume`, `/session`, `/tree`, `/name`, `/compact`, `/abort`, and `/stop` prefer the ambient topic workspace; `/workspace` itself remains the global/manual dashboard. For native ordinary topic messages in groups, keep using an operator-controlled private forum group where the bot is admin, privacy mode is disabled and the bot was re-added, or messages explicitly address the bot.

### Streaming and Telegram HTML rendering

Closed Markdown blocks stream back as rich Telegram HTML while π is generating. The growing tail stays conservative until the final rendered reply lands. Concurrent-workspace stream finalization is verified before suppressing the normal full reply fallback, so if Telegram rejects or races the final preview edit the bridge sends the complete assistant reply as a normal message instead of leaving only a partial preview. Final Telegram-originated text replies include a display-only context-usage footer such as `—\n📊 ctx 25.6K/400K 6.4%` after outbound text handlers run, so translation/redaction handlers only see the assistant answer. Long replies are split below Telegram limits without intentionally breaking HTML structures, links, code blocks, blockquotes, lists, or code fences.
