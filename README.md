...
- **`/tree`**: Open a Telegram-native active-path prompt history. It shows only user/custom prompts from the current branch; pick one, inspect it, then use `↩️ Rewind and replace this prompt`. Rewind is allowed only while π and the Telegram queue are idle. Telegram cannot prefill π's editor, so the bridge sends the selected prompt text back for copy/edit/resend.
