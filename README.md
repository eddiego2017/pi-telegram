...
- **`/session`**: Open a Telegram-native session center for the current π session: compact name/file/id/message/token/cost/context stats, latest chat preview, and active-branch chat-history pagination. History rows render as a narrow table directly in the HTML panel, show how much tool noise is hidden, and do not trigger Telegram link previews.
