...
- **`/resume`**: List previous π sessions for the current working directory with 20 mobile-friendly two-line items per page (`① age · Nmsg` followed by the summary) plus compact page-local numeric buttons. `Nmsg` counts visible user/assistant messages only, not hidden thinking or tool noise. This command only resumes sessions; deletion lives in `/delete`.
