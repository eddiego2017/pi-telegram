...
- Browser visual checkpoints created inside a tab worker read that tab's latest Telegram target from the existing tab registry, so screenshots from a background tab reply to that tab's prompt rather than following the currently active tab.
