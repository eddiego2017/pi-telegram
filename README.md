...
- **`/delete`**: Open a dedicated single-session delete picker for previous sessions in the current working directory. Pick a numeric row, confirm with an explicit `🗑 Delete` button, or go Back/Cancel. Deletion uses the `trash` CLI when available and falls back to removing the session file.
