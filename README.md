...
- **`/resume`**: List previous π sessions for the current working directory with readable 3-column body rows (`01  age|Nmsg  summary`) plus compact index buttons. Use `🗑 Delete sessions` to enter fake-checkbox delete mode, select one or more stale sessions, then confirm batch deletion.
