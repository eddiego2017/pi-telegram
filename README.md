...
- Browser visual checkpoints created inside a tab worker use that worker's per-tab Telegram target file, so screenshots from a background tab reply to that tab's prompt rather than following the currently active tab.
