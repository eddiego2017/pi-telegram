...
- **`/resume`**: List previous π sessions for the current working directory with 20 mobile-friendly two-line items per page (`① age · Nmsg` followed by the summary) plus compact page-local numeric buttons. `Nmsg` counts visible user/assistant messages only, not hidden thinking or tool noise. Use `🗑 Delete sessions` to enter fake-checkbox delete mode, select one or more stale sessions, then confirm batch deletion.
- **`/delete`**: Open a dedicated delete picker for previous sessions in the current working directory. Toggle numeric rows to select multiple sessions, use `Select page` to select the current page only, then confirm with an explicit `🗑 Delete` button. Deletion uses the `trash` CLI when available and falls back to removing the session file.
- **`/session`**: Open a Telegram-native session center for the current π session: compact name/file/id/message/token/cost/context stats, latest chat preview, direct `📜 Last 5 turns` / `📜 Full replay` buttons, and active-branch chat-history pagination. Replay resends visible user/assistant/custom chat rows from the current branch without an LLM call, hiding thinking/tool noise and adding a small `Replay msg …` header to each bubble. History rows render as a narrow HTML row table directly in the panel, show how much tool noise is hidden, and do not trigger Telegram link previews.
...
The inline application menu is the primary operator surface. It exposes status, prompt-template commands, model selection, thinking level selection, settings, queue inspection/mutation, session resume/delete flows, the `/session` session-center/history/replay view, and `/tree` session-tree rewind: a Telegram-shaped subset of the important handles normally available from the CLI. A typical control loop stays inside Telegram: open `/start`, inspect status, jump into Queue, inspect or replay current session history, rewind or branch from a previous point, delete stale work, switch model, resume or prune old sessions, return to the main menu, and keep the π session running without touching the terminal.
