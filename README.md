...
- **`/resume`**: List previous π sessions for the current working directory with mobile-friendly two-line items (`① age · Nmsg` followed by the summary) plus compact page-local numeric buttons. Use `🗑 Delete sessions` to enter fake-checkbox delete mode, select one or more stale sessions, then confirm batch deletion.
