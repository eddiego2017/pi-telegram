...
Direction change on 2026-05-23: remove browser skill automatic visual
checkpoint sends from concurrent tab support. Browser skill scripts must not
write `telegram-tabs.json`, and tab records must not store Telegram reply
targets or browser CDP targets for screenshot delivery.
